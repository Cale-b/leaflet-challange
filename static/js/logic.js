function createMap(earthquakes) {
  ...
  console.log(response);
    ...
    else if (station.geometry.coordinates[2] <= -1) {
      color = "orange";
    }
    else {
      color = 'purple';
    ...
    if (station.properties.mag < 1) {
      ...
      let Marker = L.circleMarker([station.geometry.coordinates[1],station.geometry.coordinates[0]],
       { fillOpacity: 0.75,
        ...
        fillColor: color,
        radius: radius*5})
        .bindPopup("<h3> Mag:" + station.properties.mag + "<h3><h3>Place: " + station.properties.place + "</h3>" + "<h3><h3>Depth: " + station.geometry.coordinates[2] + "</h3>");
  ...
  console.log(map);
    ...
    limits.forEach(function(limit, index) {
            div.innerHTML += 
                "<i style='background:" + limits[index] + "'></i> " + limits[index] + (limits[index + 1] ? "&ndash;" + limits[index +1] + "<br>": "+");
    });
